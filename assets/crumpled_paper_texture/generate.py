...
def create_crumpled_paper(color1, color2, filename):
    width, height = 3840, 3840
    noise = np.random.randn(height, width).astype(np.float32)
    for i in range(5):
        noise = cv2.GaussianBlur(noise, (0, 0), sigmaX=50/(2**i), borderType=cv2.BORDER_REFLECT)
    ...
    for c in range(3):
        img[..., c] = (c1[c] * (1 - tex) + c2[c] * tex).astype(np.uint8)
    
    img[..., 3] = (tex * 255).astype(np.uint8)
    
    cv2.imwrite(filename, img)

create_crumpled_paper('#FFFFFF', '#E0E0E0', 'output_v1.png')
create_crumpled_paper('#FFFACD', '#D3D3D3', 'output_v2.png')
