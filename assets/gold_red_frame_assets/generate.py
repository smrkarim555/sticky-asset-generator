...
def create_frame(primary_color, secondary_color, filename):
    ...
    p_col = hex_to_rgb(primary_color)
    s_col = hex_to_rgb(secondary_color)

    def draw_corner(is_top):
        y_offset = 0 if is_top else height - 300
        ctx.move_to(0, y_offset + (0 if is_top else 300))
        ctx.line_to(0, y_offset + (200 if is_top else 100))
        ctx.curve_to(0, y_offset + 50, 50, y_offset, 200, y_offset)
        ctx.line_to(800, y_offset)
        ctx.line_to(900, y_offset + 150)
        ctx.line_to(200, y_offset + 150)
        ctx.curve_to(100, y_offset + 150, 100, y_offset + 250, 150, y_offset + 300)
        ctx.close_path()
        ctx.set_source_rgb(*p_col)
        ctx.fill()
...
create_frame('#FF0000', '#D4AF37', 'output_v1.png')
create_frame('#0000FF', '#C0C0C0', 'output_v2.png')
