...
def create_torn_paper(width, height, color):
    surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, width, height)
    ctx = cairo.Context(surface)
    
    # Paper path - jagged edge
    ctx.move_to(0, height * 0.4)
    x_steps = 50
    for i in range(x_steps + 1):
        x = (width / x_steps) * i
        y = (height * 0.4) + (np.sin(i * 0.5) * 50) + (np.random.normal(0, 10))
        ctx.line_to(x, y)
    ...
    return np.frombuffer(surface.get_data(), np.uint8).reshape(height, width, 4)
...
v1 = create_torn_paper(width, height, (0.95, 0.95, 0.95))
save_png(v1, 'output_v1.png')
v2 = create_torn_paper(width, height, (0.8, 0.7, 0.6))
save_png(v2, 'output_v2.png')
