...
def create_scroll(width, height, color1, color2, filename):
    surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, width, height)
    ctx = cairo.Context(surface)
    
    def draw_parchment(ctx, w, h, c1, c2):
        ctx.set_source_rgba(0, 0, 0, 0)
        ctx.paint()
        
        # Main body
        ctx.move_to(300, 200)
        ctx.curve_to(300, 200, 1920, 150, 3540, 200)
        ctx.curve_to(3540, 200, 3540, 3640, 3540, 3640)
        ctx.curve_to(3540, 3640, 1920, 3690, 300, 3640)
        ctx.curve_to(300, 3640, 300, 200, 300, 200)
        
        grad = cairo.LinearGradient(0, 0, width, height)
        grad.add_color_stop_rgba(0, *c1, 1)
        grad.add_color_stop_rgba(1, *c2, 1)
        ctx.set_source(grad)
        ctx.fill()
        
        # Scrolls
        for x in [300, 3540]:
            ctx.arc(x, 1920, 300, 0, 2*3.14159)
            ctx.set_source_rgba(*c2, 1)
            ctx.fill()
            
    draw_parchment(ctx, width, height, color1, color2)
...
size = 3840
variations = [
    ((0.8, 0.7, 0.5), (0.6, 0.5, 0.3), "output_v1.png"),
    ((0.9, 0.8, 0.6), (0.7, 0.6, 0.4), "output_v2.png")
]
for c1, c2, fn in variations:
    create_scroll(size, size, c1, c2, fn)
