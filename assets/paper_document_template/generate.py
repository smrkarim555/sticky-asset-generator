...
def draw_frame(filename, bg_color, border_color):
    width, height = 3840, 2160
    surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, width, height)
    ctx = cairo.Context(surface)
    
    # Dimensions
    rect_w, rect_h = 1200, 1600
    x, y = (width - rect_w) / 2, (height - rect_h) / 2
    
    # Fill background
    ctx.set_source_rgba(bg_color[0], bg_color[1], bg_color[2], 0.95)
    ctx.rectangle(x, y, rect_w, rect_h)
    ctx.fill()
    
    # Outer frame
    ctx.set_source_rgba(border_color[0], border_color[1], border_color[2], 1.0)
    ctx.set_line_width(4)
    ctx.rectangle(x + 20, y + 20, rect_w - 40, rect_h - 40)
    ctx.stroke()
    
    # Inner frame
    ctx.set_line_width(2)
    ctx.rectangle(x + 35, y + 35, rect_w - 70, rect_h - 70)
    ctx.stroke()
    
    # Shadow
    ctx.set_source_rgba(0, 0, 0, 0.2)
    ctx.rectangle(x + 10, y + 10, rect_w, rect_h)
    ctx.stroke()
    
    surface.write_to_png(filename)

# Colors normalized
hex_to_rgb = lambda h: [int(h[i:i+2], 16)/255.0 for i in (1, 3, 5)]

draw_frame('output_v1.png', hex_to_rgb('#FFF5E6'), hex_to_rgb('#E6C28C'))
draw_frame('output_v2.png', hex_to_rgb('#F0F8FF'), hex_to_rgb('#B0C4DE'))
